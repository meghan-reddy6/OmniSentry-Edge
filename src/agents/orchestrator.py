        ...
        if self.state == SystemState.IDLE:
            ...
            if isinstance(event, MotionDoneEvent):
                ...
                if self.current_prompt and self.current_prompt.strip():
                    logger.info(f"[Orchestrator]: Resuming tracking for '{self.current_prompt}' at new acoustic orientation.")
                    await self._transition_to(SystemState.VLM_TRACKING)
                else:
                    logger.info("[Orchestrator]: Acoustic seek complete. Returning to IDLE.")
